  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      for (let i = 0; i < 60; i++) {
        ...
        traces.push({ points, progress:0, speed:0.004+Math.random()*0.008, hue:265+Math.random()*50, width:Math.random()<0.3?2:1, delay:Math.random()*0.6 });
      }
    };
    init();
    ...
    const drawTrace = (trace) => {
      ...
      const pts = trace.points, total = pts.length - 1;
      const drawn = trace.progress * total, seg = Math.floor(drawn), frac = drawn - seg;
      ctx.beginPath();
      ctx.moveTo(pts[0].x, pts[0].y);
      for (let i = 1; i <= seg && i < pts.length; i++) ctx.lineTo(pts[i].x, pts[i].y);
      if (seg < total) {
        const p0 = pts[seg], p1 = pts[seg+1];
        ctx.lineTo(p0.x+(p1.x-p0.x)*frac, p0.y+(p1.y-p0.y)*frac);
      }
      ctx.strokeStyle=`hsla(${trace.hue},70%,55%,0.55)`; ctx.lineWidth=trace.width;
      ctx.shadowColor=`hsla(${trace.hue},70%,60%,0.8)`; ctx.shadowBlur=6;
      ctx.stroke(); ctx.shadowBlur=0;
      pts.slice(0,seg+1).forEach(p=>{
        ctx.beginPath(); ctx.arc(p.x,p.y,2.5,0,Math.PI*2);
        ctx.fillStyle=`hsla(${trace.hue},80%,70%,0.9)`;
        ctx.shadowColor=`hsla(${trace.hue},80%,70%,1)`; ctx.shadowBlur=8;
    ...
    const draw = () => {
      ...
      pulses=pulses.filter(p=>{
        p.life-=0.035; const r=(1-p.life)*18;
        ctx.beginPath(); ctx.arc(p.x,p.y,r,0,Math.PI*2);
        ctx.strokeStyle=`hsla(${p.hue},80%,65%,${p.life*0.6})`; ctx.lineWidth=1.5;
        ctx.shadowColor=`hsla(${p.hue},80%,65%,0.8)`; ctx.shadowBlur=10;
        ctx.stroke(); ctx.shadowBlur=0;
        return p.life>0;
      });
    ...
    gsap.to(obj, {
      ...
      onComplete() {
        ...
        if (uiRef.current) gsap.to(uiRef.current, { opacity:0, scale:1.05, duration:0.3 });
        if (wrapRef.current) {
    ...
    return () => {
      ...
      gsap.killTweensOf(uiRef.current);
    };
  }, [onComplete]);

  // ✅ Stay in DOM but invisible — prevents removeChild crash
  if (done) return <div style={{ position:"fixed", inset:0, zIndex:-1, pointerEvents:"none", visibility:"hidden" }}/>;

  return (
    <div ref={wrapRef} style={{ position:"fixed", inset:0, zIndex:9999, background:"#000", overflow:"hidden" }}>
      <canvas ref={canvasRef} style={{ position:"absolute", inset:0, width:"100%", height:"100%" }}/>
      <div ref={uiRef} style={{ position:"absolute", inset:0, zIndex:2, opacity:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
